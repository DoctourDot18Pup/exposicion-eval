...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const rubrica = await prisma.rubrica.findUnique({
    where: { id: rubricaId },
    include: { criterios: { select: { ponderacion: true } } },
  });

  if (!rubrica) {
    return NextResponse.json(
      makeApiError(404, "Not Found", `Rúbrica con id ${rubricaId} no encontrada`, path),
      { status: 404 }
    );
  }

  const sumaActual = rubrica.criterios.reduce((acc, c) => acc + Number(c.ponderacion), 0);
  if (sumaActual + ponderacion > 100) {
    return NextResponse.json(
      makeApiError(
        400,
        "Bad Request",
        `La ponderación acumulada (${sumaActual.toFixed(2)} + ${ponderacion}) supera 100`,
        path
      ),
      { status: 400 }
    );
